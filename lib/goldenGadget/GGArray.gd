...
func map_fn(f, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	return _w_(GGI.map_fn_(_val, f, ctx))
...
func map(f, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	return _w_(GGI.map_(_val, f, ctx))
...
func map_in_mtd(inner_method_name: String, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	var r := []
	for x in _val:
		r.push_back(GGI.call_f0_w_ctx(funcref(x, inner_method_name), ctx))
	return _w_(r)
...
func map_fld(field_name: String) -> GGArray:
	var r := []
	for x in _val:
		if x && field_name in x:
			r.push_back(x[field_name])
	return _w_(r)
...
func for_each(f, ctx = GGI._EMPTY_CONTEXT) -> void:
	GGI.for_each_(_val, f, ctx)
...
func filter_fn(predicate, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	return _w_(GGI.filter_fn_(_val, predicate, ctx))


## Similar to [[filter_fn]], but supports [[FuncLike]].
## @example `G(range(-4, 4)).filter("x => x >= 2").val` returns `[2, 3]`
func filter(predicate, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	return _w_(GGI.filter_(_val, predicate, ctx))
...
func filter_in_mtd(inner_method_name: String, ctx = GGI._EMPTY_CONTEXT) -> GGArray:
	var r := []
	for x in _val:
		if GGI.call_f0_w_ctx(funcref(x, inner_method_name), ctx):
			r.push_back(x)
	return _w_(r)
...
func filter_by_fld(field_name: String) -> GGArray:
	var r := []
	for x in _val:
		if field_name in x && x[field_name]:
			r.push_back(x)
	return _w_(r)
...
func filter_by_fld_val(field_name: String, field_value) -> GGArray:
	var r := []
	for x in _val:
		if field_name in x && x[field_name] == field_value:
			r.push_back(x)
	return _w_(r)
...
func without(value_to_omit) -> GGArray:
	return _w_(GGI.without_(_val, value_to_omit))


## Return a new [[GGArray]] omitting `null` items.
## @example `G([1, null, 3]).compact().val` returns `[1, 3]`
func compact() -> GGArray:
	return _w_(GGI.compact_(_val))
...
func find_or_null(predicate, ctx = GGI._EMPTY_CONTEXT):
	return GGI.find_or_null_(_val, predicate, ctx)


## Find a first element for which predicate holds. Return `null` when no match is found.
func find(predicate, ctx = GGI._EMPTY_CONTEXT):
	return GGI.find_(_val, predicate, ctx)
...
## @param ctx {any}
## @return {int | null}
func find_index_or_null(predicate, ctx = GGI._EMPTY_CONTEXT):
	return GGI.find_index_or_null_(_val, predicate, ctx)
...
## @param ctx {any}
## @return {int}
func find_index(predicate, ctx = GGI._EMPTY_CONTEXT):
	return GGI.find_index_(_val, predicate, ctx)
...
## @return {R}
func foldl_fn(f: FuncRef, zero, ctx = GGI._EMPTY_CONTEXT):
	return GGI.foldl_fn_(_val, f, zero, ctx)


## Similar to [[foldl_fn]], but also supports [[FuncLike]].
## @example `G([1, 2, 3]).foldl("a, x => a + x", -6)` returns `0` (-6 + 1 + 2 + 3)
func foldl(f, zero, ctx = GGI._EMPTY_CONTEXT):
	return GGI.foldl_(_val, f, zero, ctx)
...
func to(f):
	return GGI.f_like_to_func(f).call_func(_val)
...
func sort_by(cmp_f: FuncRef) -> GGArray:
	return _w_(GGI.sort_by_(_val, cmp_f))


## Sort an array of objects by one given field.
func sort_by_fld(field_name: String) -> GGArray:
	return _w_(GGI.sort_by_fld_(_val, field_name))


## Sort an array using a mapping function (result is sorted on values obtained from the mapping function).
func sort_with(map_f: FuncRef) -> GGArray:
	return _w_(GGI.sort_with_(_val, map_f))
...
func zip(other: Array) -> GGArray:
	return _w_(GGI.zip_(_val, other))
...
func sample_or_null():
	return GGI.sample_or_null_(_val)
...
func reverse() -> GGArray:
	return _w_(GGI.reverse_(_val))
...
func any(p) -> bool:
	return GGI.any_(_val, p)


## Append an item to an end of the array.
func append(y) -> GGArray:
	return _w_(GGI.append_(_val, y))


## Prepend an item to a start of the array.
func prepend(y) -> GGArray:
	return _w_(GGI.prepend_(_val, y))


## Concatenate arrays (`other` to end).
func concat(other: Array) -> GGArray:
	return _w_(GGI.concat_(_val, other))


## Concatenate arrays (`other` to start)
func concat_left(other: Array) -> GGArray:
	return _w_(GGI.concat_left_(_val, other))
...
func group_with(f) -> GGArray:
	return _w_(GGI.group_with_(_val, f)).map(_w)


## Transpose a 2D matrix.
## @example `G([[1, 2], [3, 4]]).transpose()` returns `GGArray`s equivalent to `[[1, 3], [2, 4]]`
func transpose() -> GGArray:
	return _w_(GGI.transpose_(_val)).map(_w)
...
func nub() -> GGArray:
	return _w_(GGI.nub_(_val))


## Remove all duplicate items.
func uniq() -> GGArray:
	return _w_(GGI.uniq_(_val))


## Convert array of floats to array of integers. Useful for correcting parsed JSONs.
func float_arr_to_int_arr() -> GGArray:
	return _w_(GGI.float_arr_to_int_arr_(_val))
